...
- 다른 서비스에서 캐싱 되어 사용하는 데이터 중 변경 사항이 있는 경우, 이를 캐싱하여 사용하고 있는 부분에 동기화(전파) 될 방법에 대해 같이 고민되어야합니다.
- 데이터 싱크에 민감하지 않은 서비스는 싱크가 안맞아도 상관없지만, 데이터 싱크에 대한 신뢰도가 중요한 서비스는 캐싱 처리에 대한 고민이 필요합니다. (금융, 예매 등..)
- 위에서 자주 설명했던 조회 빈도나 비용과 더불어, 높은 Cache Hit Ratio와 관리 측면에서 전략에 따라 다를수 있겠지만 변경이 많지 않은 데이터가 캐싱하기에 쉬운 데이터라고 볼수 있습니다.
#### 데이터 용량
- 미사용되는 데이터에 대해 처리
...
#### 성능 비교
절대적인 성능 차이 확인을 위한 비교는 아니며, 캐싱 전후의 성능 차이 확인을 위해 간단히 비교를 진행하였습니다.  
- 조건
```text
같은 원격지의 Oracle, Redis에 호출을 수행했을때의 성능 비교 
- 동일한 메소드를 2개 생성하고, 하나의 메소드는 @Cacheable로 캐싱을 구현
- 5,000번 호출하여 평균 속도(초)를 확인

1. Oracle : 별도의 Join없이 1 Table조회하였고, 검색조건은 PK로 조회
2. Redis : Key를 Oracle PK로 사용하였고, 검색 조건은 별도의 Indexing 없이 KEY로 조회
```
- 결과
```text
1. Oracle : 평균 36초 소요
...
* 위의 성능 비교는 둘의 용도는 다르고 시스템 구성/데이터등 영향을 받는 요소가 많은 부분입니다. 구현 방법에 맞춰서 개발시 상세한 성능 테스트가 필요한 부분입니다.
* 레디스를 예를 들었지만, 이외에도 많은 캐시 솔루션이 있으며 상황에 맞는 캐시 솔루션을 판단하여 사용해야 합니다.
...
이후 아티클을 통해 레디스의 구조, 전략등에 대해 설명드리도록 하겠습니다.